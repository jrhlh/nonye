  ...
  setTimeout(() => {
    ...
    if (isTemperatureAbnormal.value) {
      suggestions.push({
        ...
        warningValue: 32,
        urgent: true,
        faultDescription: '当前温度高于阈值(30°C)',
        timestamp: new Date().toLocaleString()
      });

      // Add historical temperature anomaly - 保持不变
      suggestions.push({
        deviceName: '温度传感器',
        warningType: 'temperature_high',
        warningValue: 38,
...
const suggestionText = (suggestion: {
  ...
  switch (suggestion.warningType) {
    ...
    case 'humidity_high':
      return `${suggestion.deviceName}湿度持续异常，请检查灌溉系统`; // 新增湿度类型
    case 'ph_abnormal':
...
const simulateEnvironmentChange = () => {
  ...
  fetchDeviceATemperature();
  fetchHumidityData();
  fetchMaintenanceSuggestions(); // 每次环境变化时重新生成维护建议
  fetchPhData(); // 每次环境变化时重新获取pH值
};
...
<template>
  <div id="all">
    ...
    <div class="fixed-top-container">
      ...
      <div class="box">
        <div class="data-container">
          <div class="number" style="padding-right: 5px;">65420</div>
    ...
    <div class="body">
      <div class="body-top-box">
        ...
        <div class="body-top-temp">
          <h3 class="clickable-area" @click="drawerVisible = true">五天之内的温度柱形图报表</h3>
          <wendu2/>
        </div>
        <!-- Humidity content -->
        <div class="body-top-shidu">
          <h3 class="clickable-area01" @click="drawerVisibleHumidity = true">七天之内的湿度数据报表</h3>
          <shidu2/>
        </div>
      </div>
      <div class="body-middle-box">
        ...
        <div class="body-mid-ph">
          <h3 class="clickable-area" @click="drawerVisiblePh = true">PH值的散点分布报表</h3>
          <ph/>
        </div>
      ...
      <div class="smart-analysis-container">
        ...
        <div class="maintenance-section">
          <h3 class="section-title">建议维护措施</h3>
    ...
    >
      <div ref="pdfModalContent" class="pdf-preview-content">
        ...
        <div class="preview-body">
          ...
          <!-- Smart analysis in PDF preview -->
          <div class="smart-analysis-container1">
            <div class="analysis-header">
              <h2 class="analysis-title">智能分析</h2>
            </div>
            <div class="confidence-section">
              <h3 class="section-title" style="padding-top: 10px;">故障预测置信度</h3>
              <div class="custom-progress-bar">
                <div class="progress-background"></div>
                <div class="progress-fill" :style="{ width: progressValue + '%' }"></div>
              </div>
            ...
            <div class="maintenance-section" style="position: relative;top: -10px">
              <h3 class="section-title">建议维护措施</h3>
              <ul class="maintenance-list" v-if="!isMaintenanceLoading && maintenanceSuggestions.length > 0">
                <li
                    v-for="(suggestion, index) in maintenanceSuggestions"
